...
- [Sprint 4 검증 보고서](sprint4/playwright-report.md) — curl 자동 검증 + 코드 리뷰 결과 포함
- [deploy.md Sprint 4 섹션](../deploy.md) — 자동/수동 검증 체크리스트

## 자동 검증 결과 (요약)

- ✅ `npm run build` — 빌드 성공, TypeScript 오류 없음
- ✅ `POST /api/auth/admin` — 로그인 성공
- ✅ `GET /api/sessions/[id]/submissions` — 목록 조회, 상태 필터 동작
- ✅ `PATCH /api/sessions/[id]/submissions/[subId]` — 제외/복원 처리 정상
- ✅ `GET /api/sessions/[id]/export/csv` — CSV 다운로드, BOM 포함 확인
- ✅ 인증 없이 접근 시 401 반환 확인

## 수동 검증 항목

`docs/deploy.md` Sprint 4 섹션 참조 (모두 완료).
