...
## 수동 검증 결과 (2026-03-10 완료)

모든 수동 검증 항목이 통과되었습니다.

| 검증 항목 | 결과 |
|-----------|------|
| 관리자 로그인 (`admin1234`) → 쿠키 발급 + 대시보드 이동 | ✅ 통과 |
| 잘못된 비밀번호 → 오류 메시지 표시 | ✅ 통과 |
| 미인증 상태 `/admin/dashboard` 직접 접속 → 로그인 리디렉션 | ✅ 통과 |
| 새로고침 후 로그인 상태 유지 (쿠키 기반) | ✅ 통과 |
| 관리자 대시보드 세션 목록 실제 DB 데이터 렌더링 | ✅ 통과 |
| 세션 생성 → 목록 즉시 반영 | ✅ 통과 |
| 세션 상세 제출 목록 실제 DB 렌더링 | ✅ 통과 |
| 마감 연장 / 즉시 마감 버튼 동작 | ✅ 통과 |
| 제출 폼 세션 정보 실제 DB 로드 | ✅ 통과 |
| GitHub URL 검증 — debounce 500ms 후 스피너 표시 | ✅ 통과 |
| GitHub URL 검증 — 유효한 public URL 초록 체크 아이콘 | ✅ 통과 |
| GitHub URL 검증 — 존재하지 않는 URL 빨간 X 아이콘 | ✅ 통과 |
| 제출 폼 완료 → 완료 화면 (제출 시각, 입력 요약) 표시 | ✅ 통과 |
| 참가자 확인 페이지 이름+이메일 조회 정상 동작 | ✅ 통과 |
| 한글 이름 DB 저장 인코딩 — 브라우저에서 정상 조회 확인 | ✅ 통과 (curl 인코딩 이슈였음, 브라우저 정상) |
| 즉시 마감 후 제출 폼 접속 → "마감된 세션입니다" 오류 확인 | ✅ 통과 |

상세 수동 검증 내역은 `docs/deploy.md`의 "수동 검증 완료 (2026-03-10)" 섹션을 참조하세요.
