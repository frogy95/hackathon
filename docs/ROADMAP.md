...
- **현재 진행 단계**: Phase 1 (Sprint 1 완료, Sprint 2 예정)
- **기준일**: 2026-03-09
...
| 항목 | 상태 |
|------|------|
| 전체 진행률 | 12.5% (Sprint 1/8 완료) |
| 현재 Phase | Phase 1 (Sprint 1 완료) |
| 다음 마일스톤 | Phase 1 완료 - 프론트엔드 UI 쉘 + DB 스키마 |
...
### 🔄 상태: 진행 중 (Sprint 1 완료 / Sprint 2 예정)
...
- ✅ **T1-4. 참가자 제출 폼 UI** (복잡도: M) [FR-004]
  ...
  - 입력 필드: 이름(2자 이상), 사번(형식 검증), GitHub URL(URL 형식), 배포 URL(선택)
...
- ⬜ **T1-8. 세션 상세 - 제출 현황 UI** (복잡도: L) [FR-006, FR-007]
  ...
  - 제출 목록 테이블: 이름, 사번, 저장소 URL, 배포 URL 유무, 제출 시각, 평가 상태
...
5. `browser_fill_form` -> 이름, 사번, GitHub URL 입력
...
- ⬜ **T2-4. 참가자 제출 API** (복잡도: L) [FR-004, FR-005]
  ...
  - `GET /api/sessions/[id]/submissions/check` - 이름+사번으로 본인 제출 조회
  - GitHub URL 유효성 검증 (서버 사이드): Octokit으로 public repo 존재 여부 확인
  - 동일 사번 중복 제출 시 기존 제출 덮어쓰기 (upsert)
  - 마감일시 이후 제출 거부 로직
...
| 참가자 사칭 제출 | 낮음 | 낮음 | 사번 기반 간이 인증, 관리자 제출 목록 검토 |
