...
| 항목 | 상태 |
|------|------|
| 전체 진행률 | Sprint 11 완료 (운영 이슈 수정) |
| 현재 Phase | 운영 개선 (Sprint 11 완료 2026-03-12) |
| 다음 마일스톤 | Vercel 배포 후 실운영 검증 |
...
| 구분 | 기술 | 선정 이유 |
|------|------|-----------|
| 프론트엔드 | Next.js 16.1.6 (App Router) + TypeScript | 풀스택 단일 프레임워크, SSR/SSG 지원, API Routes 내장 |
| UI | Tailwind CSS + shadcn/ui | 빠른 UI 구축, 일관된 디자인 시스템, 접근성 기본 지원 |
| 차트 | Recharts | 레이더 차트 지원, React 네이티브 통합 |
| AI | Claude API (haiku / sonnet 선택 가능) | 코드 분석 능력, Vision 지원, 비용 효율 |
...
- 클라이언트 데이터 페칭: Next.js fetch 직접 사용 (tanstack-query/SWR 미사용)

---

## Phase 3: AI 평가 엔진 + 결과 대시보드 연결 (Sprint 5-6, 2주)

### ✅ 상태: 완료 (Sprint 5 완료 2026-03-10 / Sprint 5.1 완료 2026-03-10 / Sprint 5.2 완료 2026-03-11 / Sprint 6 완료 2026-03-11)
...
#### Sprint 6: 결과 대시보드 실데이터 연결 (6주차) — ✅ 완료 (2026-03-11)
...
- ✅ **T3-6. 참가자 결과 조회 연결** (복잡도: S) [FR-014]
  ...
  - 검증: 결과 공개 전/후 참가자 이메일+비밀번호 조회 결과 차이 확인
...
## Phase 6: 운영 이슈 수정 (Sprint 11)

### ✅ 상태: 완료 (Sprint 11 완료 2026-03-12)

### 목표
실운영 중 발견된 2가지 이슈를 수정한다: 테스트 이메일의 "결과확인" 버튼이 존재하지 않는 세션(하드코딩된 "test-session")으로 이동하는 문제, 그리고 `resultsPublished` 의존성으로 인해 평가 완료 후에도 참가자가 즉시 결과를 확인할 수 없는 문제.

### 작업 목록

#### Sprint 11: 운영 이슈 수정 — 테스트 이메일 URL + 즉시 결과 확인 (11주차) — ✅ 완료 (2026-03-12)

- ✅ **T11-1. 테스트 이메일 API sessionId 수정**: `sessionId: "test-session"` 하드코딩 → DB에서 첫 번째 세션 조회, 세션 없을 시 400 반환
- ✅ **T11-2. 백엔드 check API resultsPublished 조건 제거**: `session.resultsPublished && sub.status === "done"` → `sub.status === "done"`으로 변경
- ✅ **T11-3. 프론트엔드 submission-detail resultsPublished 조건 제거**: `resultsPublished &&` 조건 및 "결과가 아직 공개되지 않았습니다." 안내 문구 제거
- ✅ **T11-4. 이메일 안내 문구 수정**: "결과 공개 후 조회 페이지에서" → "조회 페이지에서 바로"로 변경
...
## Phase 5: 운영 준비 (Sprint 9-10)

### ✅ 상태: 완료 (Sprint 9 완료 2026-03-11 / Sprint 10 완료 2026-03-11)

### 목표
해커톤 실운영 전 필수 조건 — 내부 도메인 전용 제출 제한, 중복 제출 거부, 보너스 점수 체계 완전 제거, 자동 평가 완료 후 이메일로 결과 발송 파이프라인 구축.

### 작업 목록

#### Sprint 10: 오류 안내 개선 + 재평가 이메일 + 수정&재평가 제한 (10주차) — ✅ 완료 (2026-03-11)

- ✅ **T10-1. DB 스키마**: `submissions` 테이블에 `editCount integer` 필드 추가
- ✅ **T10-2. 타입 정의**: `src/types/index.ts` Submission 인터페이스에 `editCount: number` 추가
- ✅ **T10-3. 오류 메시지 변경**: 참가자 화면 error 상태 안내 문구 → "관리자가 오류를 확인 중이며, 처리 후 평가 결과 메일이 발송됩니다."
- ✅ **T10-4. evaluateAndNotify 리팩토링**: 평가 에러를 호출자로 전파, 이메일만 try/catch로 감쌈
- ✅ **T10-5. 일괄 평가 이메일 발송**: `runEvaluation`에서 `evaluateAndNotify` 호출로 통합
- ✅ **T10-6. 재평가 API 이메일 발송**: `re-evaluate` 라우트에서 `evaluateAndNotify` 호출
- ✅ **T10-7. POST submissions 수정&재평가**: 중복 이메일 409 거부 → editCount 확인(3회 초과 시 403) 후 UPDATE + 재평가
- ✅ **T10-8. 참가자 UI**: "수정&재평가 요청 (N/3)" 버튼 표시 (3회 초과 시 disabled)

#### Sprint 9: 운영 준비 (9주차) — ✅ 완료 (2026-03-11)

- ✅ **T9-1. 이메일 도메인 제한**: `@ubcare.co.kr` 전용 zod refine 추가 (`src/lib/validations.ts`)
- ✅ **T9-2. 중복 제출 거부**: upsert 로직 제거 → 409 Conflict 반환 (`src/app/api/sessions/[id]/submissions/route.ts`)
- ✅ **T9-3. 보너스 점수 체계 전면 제거**: `vision-evaluator.ts`, `screenshot-capturer.ts` 삭제, 관련 타입/컴포넌트/API 정리
- ✅ **T9-4. Resend 이메일 발송 모듈**: `src/lib/email-sender.ts` 신규 구현
- ✅ **T9-5. 자동 평가 + 이메일 파이프라인**: `evaluateAndNotify` 함수 + 제출 시 `waitUntil` 호출
- ✅ **T9-6. 제출 완료 UI 안내 문구**: "자동심사 후, 이메일로 평가내용을 받게됩니다." 추가
- ✅ **T9-7. 환경 변수 / 배포 설정**: `.env.example` RESEND_API_KEY/EMAIL_FROM, `vercel.json` submissions maxDuration 300

### 완료 기준 (Definition of Done)
- ✅ `@ubcare.co.kr` 외 이메일 zod 오류 반환
- ✅ 동일 이메일 중복 제출 시 409 Conflict 반환
- ✅ `bonus` 키워드 `src/db/schema.ts`, `src/db/seed.ts` 외 0건
- ✅ `email-sender.ts` 신규 구현, `evaluateAndNotify` export
...
### ✅ 상태: 완료 (Sprint 7 완료 2026-03-11 / Sprint 8 완료 2026-03-11)
...
#### Sprint 7: 배포 보너스 + 행운상 (7주차) — ✅ 완료 (2026-03-11)
...
#### Sprint 8: Vercel 실서버 배포 (8주차) — ✅ 완료 (2026-03-11)

- ✅ **T8-1. DB 드라이버 교체** — `better-sqlite3` 제거, `@libsql/client` 도입, `src/db/index.ts` libsql 재작성
- ✅ **T8-2. drizzle.config.ts** — `turso` dialect + `authToken` 인증 설정
- ✅ **T8-3. next.config.ts** — `serverExternalPackages: ["@libsql/client"]` 추가
- ✅ **T8-4. 평가 API waitUntil 래핑** — evaluate/re-evaluate 백그라운드 실행 보장
- ✅ **T8-5. vercel.json 신규 생성** — evaluate 300초, re-evaluate 60초 maxDuration 설정
- ✅ **T8-6. .env.example 업데이트** — DATABASE_URL, DATABASE_AUTH_TOKEN, DISABLE_SCREENSHOTS 환경변수 문서화
- ✅ **T8-7. docs/deploy.md** — Sprint 8 Vercel 배포 절차 추가
- ✅ **T8-8. npm run build** — TypeScript 오류 없이 빌드 성공
...
- ⏸️ 평가 기준 커스터마이징 후 평가 실행 시 반영됨 (백로그)
- ⏸️ PDF 리포트 생성 및 다운로드 가능 (백로그)
- ⏸️ Lighthouse 성능 점수 90+ (백로그)
...
| 마일스톤 | 목표일 | 실제 완료일 | 설명 | Phase |
|----------|--------|------------|------|-------|
| M1: 프론트엔드 UI 완성 | 2주차 (Sprint 2 종료) | 2026-03-10 | 모든 화면 목업 데이터로 렌더링, 사용자 피드백 수집 가능 | Phase 1 |
| M2: MVP 릴리스 | 4주차 (Sprint 4 종료) | 2026-03-10 | 제출 -> 관리 흐름 동작, CSV 내보내기, 배포 가능 | Phase 2 |
| M3: AI 평가 동작 | 6주차 (Sprint 6 종료) | 2026-03-11 | 자동 평가 + 결과 대시보드 + 참가자 결과 조회 | Phase 3 |
| M4: 핵심 기능 완성 | 8주차 (Sprint 8 종료) | 2026-03-11 | 배포 보너스 + 행운상 + Vercel 배포 | Phase 4 |
| M5: 운영 안정화 | Sprint 11 종료 | 2026-03-12 | 이메일 알림, 즉시 결과 확인, 수정·재평가 제한 | 운영 개선 |
